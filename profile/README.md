<p align="center">
  <img src="./profile/logo.png" alt="Fusion58" width="360">
</p>

<h1 align="center">Fusion58</h1>

<p align="center">
  <strong>Ingeniería con criterio. Productos que piensan por sí mismos.</strong>
</p>

<p align="center">
  <a href="https://fusion58.com">fusion58.com</a> ·
  <a href="https://route.fusion58.com">route.fusion58.com</a> ·
  <a href="mailto:hola@fusion58.com">hola@fusion58.com</a>
</p>

---

## Quiénes somos

Fusion58 es un estudio independiente de desarrollo de software con sede en Caracas, Venezuela. Diseñamos y construimos soluciones que combinan ingeniería rigurosa con productos inteligentes — equipos pequeños, foco en calidad sin compromisos.

## Qué hacemos

- **Inteligencia artificial aplicada** — LLMs, RAG, agentes conversacionales integrados a procesos de negocio.
- **Geolocalización, GPS e IoT** — tracking en tiempo real, ruteo, geocercas y telemetría de flotas.
- **Análisis y gestión de datos** — pipelines ETL/ELT, dashboards, modelos predictivos.
- **Software a medida** — aplicaciones web, móviles y APIs construidas desde cero.

## Productos propios

| Producto | Descripción |
|---|---|
| **[Route58](https://route.fusion58.com)** | Plataforma de localización y telemetría en tiempo real. Multi-activo: vehículos, personas, mascotas, activos, sensores y embarcaciones. |
| **Chat58** | Asistente conversacional con inteligencia artificial para atención al cliente. |

## Equipo fundador

| Socio | Rol | |
|---|---|---|
| Daniel Ayala | CTO | Ingeniería de software, infraestructura, producto técnico |
| Angel Pacheco | CEO | Visión, negocio, ventas, relaciones |
| Rafael Pellicer | CIO | IA, modelos, datos, RAG |

## Contacto

- Web: [fusion58.com](https://fusion58.com)
- Email: [hola@fusion58.com](mailto:hola@fusion58.com)
- Ubicación: Caracas, Venezuela
